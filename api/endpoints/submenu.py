...
async def create_submenu(
    menu_id: int, submenu: SchemaBase, session: AsyncSession = Depends(get_session)
    ):
...
async def get_single_submenu(id: int, session: AsyncSession = Depends(get_session)):
...
async def update_submenu(id: int, submenu: SchemaBase,
...
@router.delete("/{id}")
async def delete_single_submenu(id: int, session: AsyncSession = Depends(get_session)):
    single_submenu = await session.get(Submenu, id)
    if not single_submenu:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail="submenu not found"
